...
export class CategoryHttpRepository implements CategoryRepository {
  ...
  async findAll(): Promise<Category[]> {
    ...
    return response.data.map(categoryDto => this.categoryDtoToCategoryMapper.map(categoryDto))
  }
  async findByTitle(title: string): Promise<Category[]> {
    const response = await http.get<CategoryDto[]>(`/categories/?title=${title}`)
    return response.data.map(categoryDto => this.categoryDtoToCategoryMapper.map(categoryDto))
  }
  ...
  async update(id: ID, data: Partial<Category>): Promise<Category> {
    const categoryIn = await http.put<CategoryDto>(
      `/categories/${id}`,
      this.categoryToCategoryDtoMapper.mapPartial(data),
      {
        headers: {
          'Content-Type': 'application/json'
        }
